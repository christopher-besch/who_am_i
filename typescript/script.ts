...
class Person {
    ...
    private random_glare_idx: number;
    private random_border_idx: number;

    // create object from anchor
    constructor(anchor: HTMLAnchorElement) {
        this.anchor = anchor;
        this.id = this.anchor.id;
        this.data = this.anchor.getElementsByTagName(
            "person_data"
        )[0] as HTMLElement;
        this.image = this.anchor.getElementsByTagName(
            "img"
    ...
    get_name(): string {
        return this.data.getAttribute("name") as string;
    }

    get_gender(): string {
        return this.data.getAttribute("gender") as string;
    }
    ...
    is_removed(): boolean {
        if (this.data.getAttribute("removed") == "true") return true;
        return false;
    }

    is_selected(): boolean {
        if (this.data.getAttribute("selected") == "true") return true;
        return false;
    }

    set_removed_status(new_status: boolean) {
        let old_status: boolean = this.is_removed();
        if (new_status) this.data.setAttribute("removed", "true");
        else this.data.setAttribute("removed", "false");
        if (old_status != new_status) this.update();
    }

    set_selected_status(new_status: boolean) {
        let old_status: boolean = this.is_selected();
        if (new_status) this.data.setAttribute("selected", "true");
        else this.data.setAttribute("selected", "false");
        if (old_status != new_status) this.update();
    }

    toggle_removed_status(): void {
        this.set_removed_status(!this.is_removed());
    }

    toggle_selected_status(): void {
...
function set_message(id: string, msg: string): void {
    let message_box: HTMLParagraphElement = document.getElementById(
        id
    ) as HTMLParagraphElement;
    message_box.innerHTML = msg;
}

function block_end(): void {
    end_blocked = true;
    set_message("warning-box", "Das Spiel muss zurückgesetzt werden.");
}

function lift_block_end(): void {
    end_blocked = false;
...
function update_players(): void {
    people_left = 0;
    let last_person_id: string = "";
    for (let id in people) {
        // add border for selected player
        people[id].set_selected_status(
            people[id].get_id() == selected_player_id
        );
    ...
    switch (people_left) {
        case 1: {
            set_message(
                "left-box",
                `Es ist ${people[last_person_id].get_name()}!</br>
                        &#8222;Ich wusste es!&#8220;"</br>
                        &mdash;Motivationscoach Kiri`
            );
            // can't end in this case
            if (end_blocked) break;
            block_end();
            ...
            let win_modal_text: string = "";
            if (last_person_id == selected_player_id)
                win_modal_text = `Hahaha, ${people[
                    last_person_id
                ].get_name()} hat euch alle verarscht!`;
            else {
                // use correct gender
                ...
                win_modal_text = `${people[
                    selected_player_id
                ].get_name()} hat es mit deiner Hilfe geschafft, ${gender_str} bösen ${people[
                    last_person_id
                ].get_name()} nicht entdeckt zu werden!</br>
                            Good Job!`;
            }
        ...
        case 2: {
            set_message(
                "left-box",
                `Es muss nur noch eine Leut ausgeschlossen werden!</br>
                                &#8222;Fast geschafft!&#8220;</br>
                                &mdash;Motivationscoach Kiri`
            );
            break;
        ...
        default: {
            set_message(
                "left-box",
                `${people_left} Leute sind noch nicht ausgeschlossen.</br>
                                    &#8222;Du schaffst das!&#8220;</br>
                                    &mdash;Motivationscoach Kiri`
            );
            break;
...
function click_person(id: string): void {
    let person: Person = people[id];
    ...
    else person.toggle_removed_status();
    update_players();
}
...
function get_store_images(store_name: string): string[] {
    const store: HTMLDivElement = document.getElementById(
        store_name
    ) as HTMLDivElement;
    const store_images: HTMLCollectionOf<HTMLImageElement> = store.getElementsByTagName(
        "img"
    );
    // extract urls
    let images: string[] = [];
    for (let idx: number = 0; idx < store_images.length; idx++)
        images.push(store_images[idx].src);
    return images;
}

function get_people(): { [id: string]: Person } {
    let people: { [id: string]: Person } = {};
    // get all people anchors
    let people_div: HTMLDivElement = document.getElementById(
        "people"
    ) as HTMLDivElement;
    let people_anchors: HTMLCollectionOf<HTMLAnchorElement> = people_div.getElementsByTagName(
        "a"
    );

    // create Person objects
    for (let idx: number = 0; idx < people_anchors.length; idx++)
        people[people_anchors[idx].id] = new Person(people_anchors[idx]);
    return people;
}

// global variables
const borders: string[] = get_store_images("border-store");
const glares: string[] = get_store_images("glare-store");
let people: { [id: string]: Person };
let select_new_player: boolean;
let selected_player_id: string;
let people_left: number;
// when true, the game can't end
let end_blocked: boolean;

///////////////////
// modal control //
///////////////////

function open_modal(modal_id: string) {
...
function close_modal(modal_id: string) {
    let modal: HTMLDivElement = document.getElementById(
        modal_id
    ) as HTMLDivElement;
    // remove backdrop
    ...
    let buttons: HTMLCollectionOf<HTMLButtonElement> = modal.getElementsByTagName(
        "button"
    );
    for (let idx: number = 0; idx < buttons.length; idx++)
        if (buttons[idx].classList.contains("modal-default-hide"))
            buttons[idx].hidden = true;
}

// get the modal
let win_modal: HTMLDivElement = document.getElementById(
    "win-modal"
) as HTMLDivElement;
